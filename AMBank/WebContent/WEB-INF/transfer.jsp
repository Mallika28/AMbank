...
function editrecord(name){
    var f=document.fi;
    f.method="post";
    f.action='transfer1.jsp?name='+name;
    f.submit();
}
</script>
<title>AM Bank</title>
...
function demo(){
	...
	if (document.fi.to.value == "",document.fi.amount.value == ""){
		alert ( "Please enter all  details.");

		return false;
		}

if (document.fi.amount.value == ""){
alert ( "Please enter the amount." );

return false;
}
if (document.fi.to.value == ""){
...
while(rs.next()){
%>
<center><h2>Transfer from <%=rs.getString("name")%>'s Account</h2></center>
<form name="fi" method="post" action="transfer1.jsp" onsubmit="return demo();" >
...
<tr><td  colspan=3 align="center"  ><input type="submit" value="Transer" style="background:gray;color:white;padding:10px;" onclick="editrecord(<%=rs.getString("name")%>);" ></td></tr>
